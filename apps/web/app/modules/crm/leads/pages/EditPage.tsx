...
import { ArrowLeft, Save, User, Phone, Mail, DollarSign, Target, Building2 } from 'lucide-react';
...
export const EditPage: React.FC = () => {
  ...
  const populateFields = (lead: ILead) => {
    setName(lead.name || '');
    setEmail(lead.email || '');
    setPhone(lead.phone || '');
    setAvatarUrl(lead.avatarUrl);
    setSource(lead.source || 'INSTAGRAM');
    setStage(lead.stage || 'NEW_INQUIRY');
    setPriority(lead.priority || 'HOT');
    setFitnessGoal(lead.fitnessGoal || 'WEIGHT_LOSS');
    setTargetBudgetMonthly(String(lead.targetBudgetMonthly || 149));
    setPreferredTimeSlot(lead.preferredTimeSlot || 'EVENING_PEAK');
    setAssignedAgent(lead.assignedAgent || 'Alex Vance');
    if (lead.branchId) setBranchId(lead.branchId);
    setNotes(lead.notes || '');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (stored) {
        ...
        if (index >= 0) {
          customList[index] = { ...customList[index], ...updatedLead } as ILead;
          localStorage.setItem('gymflow_custom_leads', JSON.stringify(customList));
        } else {
          customList.unshift({ id: id || 'LEAD-901', ...updatedLead } as ILead);
          localStorage.setItem('gymflow_custom_leads', JSON.stringify(customList));
        }
      }
      ...
      const token = localStorage.getItem(STORAGE_KEYS.AUTH_TOKEN);
      await fetch(`https://gymflow-api-2jdh.onrender.com/api/v1/crm/leads/${id}`, {
        method: 'PUT',
        headers: {
          Authorization: token ? `Bearer ${token}` : '',
          'Content-Type': 'application/json',
      ...
      toast.success(`Prospect Lead "${name}" updated successfully!`);
      navigate('/crm/leads');
    } catch {
  ...
  return (
    <PageContainer>
      <PageHeader
        title={`Edit Prospect: ${name}`}
        subtitle={`Modify lead qualification and pipeline parameters for #${id || 'LEAD-901'}`}
        actions={
            ...
            <CardFooter className="flex items-center justify-between border-t border-border pt-4 bg-muted/20">
              ...
              <div className="flex gap-2.5">
                ...
                <Button type="submit" loading={loading} className="gap-1.5 shadow-sm">
                  ...
                  <span>Update Prospect Lead</span>
                </Button>
              </div>
            </CardFooter>
          </Card>
