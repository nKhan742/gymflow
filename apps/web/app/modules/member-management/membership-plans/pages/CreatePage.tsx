...
export const CreatePage: React.FC = () => {
  ...
  const [code, setCode] = useState(`PLAN-${Math.floor(100 + Math.random() * 900)}`);
  const [tier, setTier] = useState('VIP_PLATINUM');
  const [description, setDescription] = useState('All-inclusive premium fitness access with unlimited classes and sauna amenities.');
  ...
  const [maxFreezeDays, setMaxFreezeDays] = useState<number>(30);
  const [popular, setPopular] = useState<boolean>(false);
  const [status, setStatus] = useState('active');

  // Inclusions List
  const [inclusions, setInclusions] = useState<string[]>([
    '24/7 Access to Gym Floor & Cardio Zone',
    'Unlimited Group Classes (Spin, HIIT, Yoga)',
    'Sauna, Steam Room & Hydro-Spa Access',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('https://gymflow-api-2jdh.onrender.com/api/v1/member-management/membership-plans', {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        toast.success(`Membership Plan "${name}" published successfully!`);
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        subtitle="Configure a new subscription tier, pricing structure, access permissions, and amenity inclusions."
        actions={
          <Button
            variant="outline"
            size="sm"
            ...
            <div className="sticky top-20 space-y-4">
              ...
              <Card className="border border-border p-4 space-y-3">
                ...
                >
                  ...
                  <span>{loading ? 'Publishing Plan...' : 'Publish Membership Plan'}</span>
                </Button>
                <Button
                  type="button"
                  variant="outline"
