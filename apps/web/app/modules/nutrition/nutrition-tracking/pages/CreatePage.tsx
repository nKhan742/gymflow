...
export const CreatePage: React.FC = () => {
  ...
  const [coachFeedback, setCoachFeedback] = useState('Target macros achieved within optimal variance limits.');
  const [reviewedByCoachName, setReviewedByCoachName] = useState('Dr. Marcus Vance, PhD, RD');
  const [branchId, setBranchId] = useState('ALL');

  // Logged Meals
  const [loggedMeals, setLoggedMeals] = useState<ILoggedMealItem[]>([
    { mealSlot: 'Breakfast', foodName: 'Overnight Oats with Whey Protein & Blueberries', timeLogged: '08:00 AM', calories: 600, proteinGrams: 45, carbsGrams: 75, fatsGrams: 14 },
    { mealSlot: 'Lunch', foodName: 'Sous-Vide Chicken Breast with Jasmine Rice', timeLogged: '01:00 PM', calories: 750, proteinGrams: 60, carbsGrams: 90, fatsGrams: 15 },
    { mealSlot: 'Dinner', foodName: 'Wild Salmon with Sweet Potato Mash & Asparagus', timeLogged: '07:30 PM', calories: 850, proteinGrams: 55, carbsGrams: 95, fatsGrams: 25 },
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newId = `LOG-${memberId.replace('MEM-', '')}-${Math.floor(100 + Math.random() * 900)}`;
    ...
    try {
      ...
      await fetch('https://gymflow-api-2jdh.onrender.com/api/v1/nutrition/nutrition-tracking', {
        method: 'POST',
        headers: {
      ...
      toast.success(`Food diary for "${memberName || 'Athlete'}" recorded!`);
      navigate(`/nutrition/nutrition-tracking/${newId}`);
    } catch {
      toast.error('Error recording nutrition log');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Log Daily Food Diary & Macros"
        subtitle="Record member meals, calculate calorie and protein adherence, and provide clinical coach oversight feedback."
        actions={
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{loading ? 'Submitting...' : 'Save Daily Diary'}</span>
            </Button>
          </div>
        }
      />
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Card 1: Athlete & Targets */}
        <Card className="border-border/80 shadow-sm">
          <CardHeader className="pb-4">
            <CardTitle className="text-base flex items-center gap-2">
              <Activity className="h-4 w-4 text-primary" />
              Athlete Identification & Target Budgets
            </CardTitle>
            <CardDescription className="text-xs">
              Assign member, date, target macro budget, and adherence classification.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {/* Target vs Consumed Table */}
            <div className="space-y-2 pt-2 border-t border-border/80">
              <label className="text-xs font-semibold text-foreground">Target vs. Consumed Macro Breakdown</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-foreground">Logged Water Intake (Liters)</label>
                <Input
                  type="number"
                  step="0.1"
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-foreground">Reviewing Coach / Nutritionist</label>
                <Input
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-foreground">Coach Guidance & Audit Commentary</label>
              <textarea
                value={coachFeedback}
        ...
        <Card className="border-border/80 shadow-sm">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-xs">
              Chronological food diary entries submitted by athlete.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-muted-foreground">Logged Entries</span>
