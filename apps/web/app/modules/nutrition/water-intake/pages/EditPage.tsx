...
export const EditPage: React.FC = () => {
  ...
  const loadLog = () => {
    setFetching(true);
    let matched: IWaterIntakeLog | undefined;
    ...
    if (!matched) {
      matched = DEFAULT_WATER_LOGS.find((l) => l.id === id || l.code === id);
    }
    ...
    if (matched) {
      ...
      setTargetVolumeMl(String(matched.targetVolumeMl));
      setConsumedVolumeMl(String(matched.consumedVolumeMl));
      setHydrationStatus(matched.hydrationStatus);
      setElectrolyteScorePercent(String(matched.electrolyteScorePercent || 95));
      setSweatLossReplenishedMl(String(matched.sweatLossReplenishedMl || 1200));
      setSodiumMg(String(matched.sodiumMg || 1000));
      setPotassiumMg(String(matched.potassiumMg || 600));
      setMagnesiumMg(String(matched.magnesiumMg || 300));
      setBranchId(matched.branchId || 'ALL');
      setNotes(matched.notes || '');
      setHourlyLogs(matched.hourlyLogs || []);
    }
    setFetching(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch(`https://gymflow-api-2jdh.onrender.com/api/v1/nutrition/water-intake/${logId}`, {
        method: 'PUT',
        headers: {
      ...
      toast.success(`Hydration diary for "${memberName}" updated!`);
      navigate(`/nutrition/water-intake/${logId}`);
    } catch {
      toast.error('Error updating water intake log');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageContainer>
      <PageHeader
        title={`Edit Water Diary: ${memberName || id}`}
        subtitle="Modify logged fluid amounts, timing slots, and mineral concentrations."
        actions={
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/nutrition/water-intake/${id}`)}
            >
        ...
        <Card className="border-border/80 shadow-sm">
          <CardHeader className="pb-4">
            <CardTitle className="text-base flex items-center gap-2">
              ...
              Hydration Identity & Volumes
            </CardTitle>
            <CardDescription className="text-xs">
              Daily target volume and hydration status classification.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-muted/30 p-3 rounded-lg border border-border/60">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-muted-foreground">Target Fluid (ml)</label>
                <Input
                  type="number"
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-foreground">Hydration Notes</label>
              <textarea
                value={notes}
