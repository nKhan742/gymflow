...
export const EditPage: React.FC = () => {
  ...
  const populateFields = (item: INotificationPreferenceModel) => {
    setUserName(item.userName || '');
    setUserEmail(item.userEmail || '');
    setUserAvatar(item.userAvatar);
    setInAppPushEnabled(item.inAppPushEnabled ?? true);
    setEmailDigestEnabled(item.emailDigestEnabled ?? true);
    setEmailCadence(item.emailCadence || 'INSTANT');
    setSmsUrgentAlertsEnabled(item.smsUrgentAlertsEnabled ?? true);
    setWhatsappDispatchEnabled(item.whatsappDispatchEnabled ?? true);
    setTurnstileSecurityAlerts(item.turnstileSecurityAlerts ?? true);
    setPtBookingReminders(item.ptBookingReminders ?? true);
    setInvoicePaymentReceipts(item.invoicePaymentReceipts ?? true);
    setEmergencySosAlerts(item.emergencySosAlerts ?? true);
    setQuietHoursStart(item.quietHoursStart || '22:00');
    setQuietHoursEnd(item.quietHoursEnd || '07:00');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const updatedPreference: Partial<INotificationPreferenceModel> = {
      userName,
      userEmail,
      userAvatar,
      inAppPushEnabled,
    ...
    try {
      ...
      if (stored) {
        ...
        if (index >= 0) {
          customList[index] = { ...customList[index], ...updatedPreference } as INotificationPreferenceModel;
          localStorage.setItem('gymflow_custom_notification_preferences', JSON.stringify(customList));
        } else {
          customList.unshift({ id: id || 'NOTIF-101', ...updatedPreference } as INotificationPreferenceModel);
          localStorage.setItem('gymflow_custom_notification_preferences', JSON.stringify(customList));
        }
      }

      const token = localStorage.getItem(STORAGE_KEYS.AUTH_TOKEN);
      await fetch(`https://gymflow-api-2jdh.onrender.com/api/v1/profile/profile-notifications/${id}`, {
        method: 'PUT',
        headers: {
          Authorization: token ? `Bearer ${token}` : '',
          'Content-Type': 'application/json',
      ...
      toast.success(`Notification preferences #${id} updated!`);
  ...
  return (
    <PageContainer>
      <PageHeader
        title={`Edit Notifications: ${userName || 'Staff Member'}`}
        subtitle={`Modify dispatch gateways, quiet hours, and automated event triggers for #${id || '101'}`}
        actions={
