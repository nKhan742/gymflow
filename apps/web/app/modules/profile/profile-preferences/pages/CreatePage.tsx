...
export const CreatePage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newId = `PREF-${Math.floor(100 + Math.random() * 900)}`;

    const newPreference: IAppPreferenceModel = {
      id: newId,
      _id: newId,
      userName,
      userEmail,
    ...
    try {
      ...
      const customList: IAppPreferenceModel[] = stored ? JSON.parse(stored) : [];
      const filtered = customList.filter((item) => item.id !== newId && item._id !== newId);
      filtered.unshift(newPreference);
      localStorage.setItem('gymflow_custom_app_preferences', JSON.stringify(filtered));
      ...
      toast.success(`Workspace preferences saved for "${userName}"!`);
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Configure App & Workspace Preferences"
        subtitle="Personalize UI dark/light themes, locale formatting, currency symbols, default dashboards, and table layout density."
        actions={
