...
export const CreatePage: React.FC = () => {
  ...
  // Form State
  const [reportTitle, setReportTitle] = useState('Monthly Turnstile Footfall & Peak Hours Capacity Audit');
  const [reportingPeriod, setReportingPeriod] = useState<IAttendanceReport['reportingPeriod']>('MONTHLY');
  const [startDate, setStartDate] = useState('2026-08-01');
  const [endDate, setEndDate] = useState('2026-08-31');
  const [totalCheckIns, setTotalCheckIns] = useState(14820);
  const [uniqueMembers, setUniqueMembers] = useState(1850);
  const [peakHour, setPeakHour] = useState('17:30 - 19:30 (Evening Peak)');
  const [peakHeadcount, setPeakHeadcount] = useState(142);
  const [averageDurationMinutes, setAverageDurationMinutes] = useState(68);
  const [groupClassAttendance, setGroupClassAttendance] = useState(3450);
  const [turnstileScanPassRate, setTurnstileScanPassRate] = useState(99.6);
  const [auditedBy, setAuditedBy] = useState('Marcus Sterling (Operations Director)');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newId = `ATT-${Math.floor(100 + Math.random() * 900)}`;

    const newReport: IAttendanceReport = {
      id: newId,
      _id: newId,
      reportTitle,
      reportingPeriod,
      ...
      auditedBy,
      auditorAvatar: auditorAvatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80',
      status,
      branchId,
    ...
    try {
      ...
      await fetch('https://gymflow-api-2jdh.onrender.com/api/v1/reports/attendance-reports', {
        method: 'POST',
        headers: {
      ...
      toast.success(`Attendance report compiled: "${reportTitle}"!`);
      navigate('/reports/attendance-reports');
    } catch {
      toast.error('Failed to compile attendance report');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Generate Attendance & Footfall Audit"
        subtitle="Compile turnstile biometric logs, peak gym rush hours, and group studio utilization telemetry."
        actions={
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                <div className="space-y-1.5">
                  ...
                  <Input
                    placeholder="e.g. 17:30 - 19:30 (Evening Peak)"
                    value={peakHour}
