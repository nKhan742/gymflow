...
import { ArrowLeft, Save, Landmark, Calendar, DollarSign, Building2, ShieldCheck, TrendingUp, PieChart } from 'lucide-react';
...
export const CreatePage: React.FC = () => {
  ...
  // Form State
  const [reportTitle, setReportTitle] = useState('Monthly Executive P&L, EBITDA & Fiscal Health Statement');
  ...
  const [operatingExpenses, setOperatingExpenses] = useState(38400);
  const [payrollExpenses, setPayrollExpenses] = useState(48200);
  const [facilitiesRentLease, setFacilitiesRentLease] = useState(19500);
  const [auditedBy, setAuditedBy] = useState('Julian Vance, Chief Financial Officer');
  ...
  const netProfit = Math.round(ebitda * 0.79); // Net after depreciation & corporate taxes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newReport: IFinanceReport = {
      id: newId,
      _id: newId,
      reportTitle,
      reportingPeriod,
      ...
      auditedBy,
      auditorAvatar: auditorAvatar || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80',
      status,
      branchId,
    ...
    try {
      ...
      await fetch('https://gymflow-api-2jdh.onrender.com/api/v1/reports/finance-reports', {
        method: 'POST',
        headers: {
      ...
      toast.success(`Executive financial report compiled: "${reportTitle}"!`);
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Generate Executive Financial & P&L Statement"
        subtitle="Compile revenue, operating expenses, payroll burn, EBITDA margin yields, and corporate net profit after tax."
        actions={
