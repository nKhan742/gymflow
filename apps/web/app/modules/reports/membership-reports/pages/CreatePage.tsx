...
import { ArrowLeft, Save, UserCheck, Calendar, TrendingUp, Building2, ShieldCheck, HeartHandshake } from 'lucide-react';
...
export const CreatePage: React.FC = () => {
  ...
  // Form State
  const [reportTitle, setReportTitle] = useState('Monthly Member Retention, Churn & Lifecycle Cohort Audit');
  const [reportingPeriod, setReportingPeriod] = useState<IMembershipReport['reportingPeriod']>('MONTHLY');
  const [startDate, setStartDate] = useState('2026-08-01');
  const [endDate, setEndDate] = useState('2026-08-31');
  const [activeMembers, setActiveMembers] = useState(1950);
  const [newSignups, setNewSignups] = useState(148);
  const [renewals, setRenewals] = useState(380);
  const [cancellations, setCancellations] = useState(32);
  const [frozenMemberships, setFrozenMemberships] = useState(45);
  const [retentionRate, setRetentionRate] = useState(95.4);
  const [churnRate, setChurnRate] = useState(2.3);
  const [auditedBy, setAuditedBy] = useState('Sarah Jenkins (Head of Member Experience)');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newId = `MBR-${Math.floor(100 + Math.random() * 900)}`;

    const newReport: IMembershipReport = {
      id: newId,
      _id: newId,
      reportTitle,
      reportingPeriod,
      ...
      churnRate,
      auditedBy,
      auditorAvatar: auditorAvatar || 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=150&auto=format&fit=crop&q=80',
      status,
      branchId,
    ...
    try {
      ...
      await fetch('https://gymflow-api-2jdh.onrender.com/api/v1/reports/membership-reports', {
        method: 'POST',
        headers: {
      ...
      toast.success(`Membership report compiled: "${reportTitle}"!`);
      navigate('/reports/membership-reports');
    } catch {
      toast.error('Failed to compile membership report');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Generate Member Lifecycle & Retention Audit"
        subtitle="Compile contract renewal health, cohort churn metrics, new sign-up velocity, and membership freeze states."
        actions={
