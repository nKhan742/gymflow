...
export const PlatformAdminProtectedRoute: React.FC<IPlatformAdminProtectedRouteProps> = ({ children }) => {
  ...
  // If currently logged into a gym workspace as a tenant user (whose role is NOT SUPER_ADMIN)
  // and NOT explicitly authenticated as the platform root owner, strictly DENY access:
  if (user && user.role !== 'SUPER_ADMIN' && (!isPlatformAuthenticated || !platformUser?.isPlatformAdmin)) {
    return (
  ...
  // If no platform root session exists, redirect directly to platform login
  if (!isPlatformAuthenticated || !platformUser?.isPlatformAdmin) {
